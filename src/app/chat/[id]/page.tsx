...
export default function ChatPage({ params }: ChatPageProps) {
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-[#fafafa] dark:bg-background">
      <Header title={chatData?.chat_title ?? ""} className="bg-[#fafafa] dark:bg-background" />

      {isLoading && <TopLinearLoader />}

      <main className={`flex-1 bg-[#fafafa] dark:bg-background ${isMainView ? "p-6 overflow-y-auto" : "overflow-hidden"}`}>
